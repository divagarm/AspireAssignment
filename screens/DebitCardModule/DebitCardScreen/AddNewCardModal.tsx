...
const generateRandomExpiryDate = () => {
  ...
  const year = String(new Date().getFullYear() + Math.floor(Math.random() * 6)).slice(-2);
  return `${month}/${year}`;
};
...
const AddNewCardModal: React.FC<AddNewCardModalProps> = ({ addNewCardModalVisible, setAddNewCardModalVisible }) => {
            ...
            <View style={styles.modalBackground}>
                <View style={styles.modalContainer}>
                    ...
                    <TouchableOpacity
                        style={[
                        styles.saveButton,
                        (cardNumber.length === 16 && cardHolderName.length > 0) && styles.saveButtonEnabled,
                        ]}
                        onPress={() => {
                            dispatch(addNewDebitCard({
                        ...
                        disabled={(cardNumber.length === 16 && cardHolderName.length > 0) ? false : true}
                        activeOpacity={0.7}
                    >
                        <Text
                        style={[
                            styles.saveButtonText,
                            (cardNumber.length === 16 && cardHolderName.length > 0) && styles.saveButtonTextEnabled,
                        ]}
                        >
                        Save
                        </Text>
