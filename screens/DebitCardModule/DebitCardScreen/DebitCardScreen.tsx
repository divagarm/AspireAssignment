...
import { updateFreezeCardData, updateWeeklySpendingLimitData } from '../../../redux/actions/debitCardModuleActions';
...
const DebitCardScreen: React.FC<DebitCardScreenProps> = () => {
  ...
  const [currentSelectedCardId, setCurrentSelectedCardId] = React.useState(debitCardData?.[0]?.id);

  const currentSelectedCard = React.useMemo(() => {
    return debitCardData?.find(card => card?.id === currentSelectedCardId) || debitCardData?.[0];
  }
  ...
  const onSetSpendingLimit = (toggleValue: boolean) => { 
    if(currentSelectedCard?.weeklySpendLimit?.limit !== 0) {
      ...
      navigation.navigate('SelectSpendingLimit', { cardDetails: currentSelectedCard });
    }
  };

  const onFreezeCard = (toggleValue: boolean) => { 
    console.log("xxx onFreezeCard", toggleValue);
    dispatch(updateFreezeCardData({
      cardID: currentSelectedCard.id,
      enabled: toggleValue
     }));
  };
  ...
  return (
    ...
    <SafeAreaView style={debitCardScreenStyles.bottomSafeAreaView}>
      ...
      <View style={{flex:1}}>
        ...
        <View style = {{ backgroundColor: '#0C365A', position: 'absolute', height: '35%', top: 0, left: 0, right: 0, zIndex: 1}}>
          {/* Header */}
          ...
          <View style={debitCardScreenStyles.balanceSection}>
            ...
            <View style={debitCardScreenStyles.balanceContainer}>
              <View style={debitCardScreenStyles.currencyBadge}>
                <Text style={debitCardScreenStyles.currencyText}>{currentSelectedCard?.currency}</Text>
              </View>
              <Text style={debitCardScreenStyles.balanceAmount}>{currentSelectedCard?.balance?.toLocaleString()}</Text>
            </View>
          </View>
        </View>
        ...
        showsVerticalScrollIndicator={false}>
          <View style={{alignItems: 'center'}}>
            <Carousel
            data = {debitCardData}
            renderItem = {({ item: currentSelectedCard, index }: { item: any, index: number}): React.Node => (
              <>
                ...
                <View style={debitCardScreenStyles.showCardContainer}>
                  <TouchableOpacity 
                    ...
                    activeOpacity={0.7}
                  >
                    <Feather name={showCardNumber ? "eye-off" : "eye"} size={16} color="#4CAF50" />
                    <Text style={debitCardScreenStyles.showCardText}>{showCardNumber ? "Hide card number" : "Show card number"}</Text>
                  </TouchableOpacity>
                </View>
                ...
                {/* Radius white background */}
                 <View style={debitCardScreenStyles.cardAbsoluteContainer}>
                  ...
                  <View style={[debitCardScreenStyles.card, { backgroundColor: currentSelectedCard?.freezeCard?.enabled ? '#6ae6a7' : '#01D167'}]}>
                    ...
                    <Text style={debitCardScreenStyles.cardholderName}>{currentSelectedCard?.holderName}</Text>
                    
                    <View style={debitCardScreenStyles.cardNumberContainer}>
                      <Text style={debitCardScreenStyles.cardNumber}>
                        {formatCardNumber(currentSelectedCard?.cardNumber, !showCardNumber)}
                      </Text>
                    </View>
                    
                    <View style={debitCardScreenStyles.cardFooter}>
                      <View>
                        <Text style={debitCardScreenStyles.cardLabel}>Thru: {currentSelectedCard?.expiryDate}</Text>
                      </View>
                      <View>
                        <Text style={debitCardScreenStyles.cardLabel}>
                          CVV: {showCardNumber ? currentSelectedCard?.cvv : '***'}
                        </Text>
                      </View>
          ...
          <View style={debitCardScreenStyles.bottomContainer}>
            ...
            {
              (currentSelectedCard?.weeklySpendLimit?.enabled && currentSelectedCard?.weeklySpendLimit?.limit) && 
              (
                <View style={debitCardScreenStyles.limitContainer}>
                  ...
                  <View style={debitCardScreenStyles.progressBarContainer}>
                    <View style={[ debitCardScreenStyles.progressBarFill, { width: `${(parseInt(currentSelectedCard?.balance) / parseInt(currentSelectedCard?.weeklySpendLimit?.limit)) * 100}%`}]} />
                  </View>
                </View>
              )
            }
            ...
            <View style={debitCardScreenStyles.menuContainer}>
              {menuItems.map((item) => {
                ...
                let toggled;
                if(item.toggleable) {
                    toggled = currentSelectedCard?.[item.id]?.enabled;
                    if (item.id === 'weeklySpendLimit') {
                        title = toggled ? item.inverseTitle : item.title;
                        subtitle = toggled ? item.inverseSubtitle + currentSelectedCard?.weeklySpendLimit?.limit : item.subtitle;
